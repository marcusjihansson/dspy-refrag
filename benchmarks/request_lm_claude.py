...
class ClaudeLM:
    """
    Client for Claude via OpenRouter (OpenAI-compatible API).
    ...
    def __init__(
        ...
        self.model = model or os.getenv("CLAUDE_MODEL", "anthropic/claude-3-haiku")
        self.base_url = base_url or os.getenv("OPENROUTER_BASE_URL", "https://openrouter.ai/api/v1")
        self.api_key = (
            api_key or os.getenv("OPENROUTER_API_KEY") or os.getenv("OPENROUTER_MONEY_KEY")
        )
        if not self.model:
            raise ValueError("CLAUDE_MODEL must be set via parameter or environment variable.")
    ...
    def __call__(self, prompt: str) -> RequestLMResponse:
        ...
        raise RuntimeError(
            f"ClaudeLM failed after {self.max_retries} attempts: {last_err}"
        )
