...
class RequestLM:
    """
    Minimal HTTP client for GitHub Models API chat completions.
    ...
    def __init__(
        ...
        self.model = model or os.getenv("GITHUB_MODEL") or os.getenv("MODEL_NAME")
        self.base_url = base_url or os.getenv("GITHUB_BASE_URL")
        self.api_key = api_key or os.getenv("GITHUB_TOKEN")
        if not self.model:
            raise ValueError(
                "GITHUB_MODEL or MODEL_NAME must be set via parameter or environment variable."
            )
        if not self.base_url:
            raise ValueError(
                "GITHUB_BASE_URL must be set via parameter or environment variable."
            )
        if not self.api_key:
            raise ValueError(
                "API key not provided. Set GITHUB_TOKEN environment variable or pass api_key parameter."
            )
...
if __name__ == "__main__":
    ...
    try:
        lm = RequestLM(
            model=os.getenv("GITHUB_MODEL") or os.getenv("MODEL_NAME"),
